...
def compute_nvi(d: pd.DataFrame, baseline_days: int = 10) -> pd.DataFrame:
    ...
    # exp scaling keeps it bounded and nice for UI
...
def build_explanations(d: pd.DataFrame, cps: list[int], baseline_days: int = 10) -> list[dict]:
    ...
    for cp in cps[:2]:  # keep 1–2 explanations
        ...
        severity = "low"
        if top_val > 25: severity = "medium"
        if top_val > 60: severity = "high"
...
# --------------------------
# 6) Build results.json (data contract)
# --------------------------
...
def main():
    ...
    result = {
        ...
        "features": {
            ...
            "backspace_rate": to_timepoints(scored, "bs_mean"),  # daily smoothed correction frequency
        },
        "explanations": build_explanations(scored, cps_idx, baseline_days=baseline_days),
    }
