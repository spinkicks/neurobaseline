...
type Explanation = { t: string; severity: string; text: string; tag?: string };
...
type Results = {
  ...
  ml?: Point[]; // optional: in case you want to chart it later
};
...
export default function Home() {
  ...
  return (
    <div style={{ padding: 24, display: "grid", gap: 16 }}>
      <h1 style={{ fontSize: 28, margin: 0 }}>NeuroBaseline — NVI Dashboard</h1>
              ...
              data.explanations.map((ex, idx) => (
                ...
                >
                  ...
                  <div style={{ marginTop: 6 }}>
                    {ex.tag === "ml" && <b>ML signal: </b>}
                    {ex.text}
                  </div>
                </div>
              ))
            ) : (
              <div style={{ opacity: 0.8 }}>No explanations found in results.json.</div>
            )}
          </div>
        </div>
      </div>
